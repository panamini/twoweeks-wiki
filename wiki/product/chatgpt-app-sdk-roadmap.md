...
The ChatGPT/App SDK roadmap is now a current checkpoint page, not just a future plan. The active source truth reaches PR316 at `application-os-foundation@bbd96b5c`, after the PR305 durable private-beta connector proof, PR306-PR311 operational and protocol hardening, and PR313-PR316 digest-based private-beta readiness work. The public credential-free canary is green, but deployment-environment digest migration and fresh authenticated ChatGPT proof after that migration are not done. Public launch and every provider/write surface remain separately gated.
...
PR313 is merged as `d091c066edceaa60ba4e92dd0e215a913aa1d2a3` from reviewed head `894ee870c24433b3636f9cfe6841a29a694e085b`. PR314 is merged as `ccbba0a4a655af942b21c8e9144c432df79cbb38` from reviewed head `85b8b201f2cae99b09007013398d3c7983220b03`. PR315 is merged as `14e5abcdc880ef9ed020fe34e3d5f586819381c4` from reviewed head `8f58c4637b81e45c6d5d8dec75bd9a2b7ff142e3`. PR316 is merged, and is the current target, as `bbd96b5cbaa3f7a24908ed51b001183b62119001` from reviewed head `0c63c234f004ac4fbd853eb6aebf328ebf6bc758`.

At this target, the canonical private-beta eligibility key is `MCP_OAUTH_PRODUCTION_PRIVATE_BETA_SUBJECT_DIGESTS`, containing lowercase SHA-256 digests. Legacy raw-subject configuration is not a supported recommendation. A fresh credential-free public smoke after all merges passed authorization-server metadata, protected-resource metadata, both supported MCP protocol versions, the exact six-tool inventory, unauthenticated `tools/call` fail-closed behavior, and malformed-token fail-closed behavior. This does not prove that the PR314 digest configuration has been migrated or deployed, and it does not replace fresh authenticated ChatGPT `tools/list` and `tools/call` proof after migration; both remain **NOT DONE**.
...
| PR313-PR316 | Digest-based private-beta readiness | merged through current target `bbd96b5cbaa3f7a24908ed51b001183b62119001`; canonical eligibility config is lowercase SHA-256 values under `MCP_OAUTH_PRODUCTION_PRIVATE_BETA_SUBJECT_DIGESTS`, and the post-merge credential-free public smoke is green; deployment env migration and fresh authenticated ChatGPT proof remain not done |
...
- [x] PR313-PR316 are merged through current target `bbd96b5cbaa3f7a24908ed51b001183b62119001`; the source contract uses lowercase SHA-256 subject digests and the fresh public no-credential canary is green.
- [ ] Migrate the deployment environment to `MCP_OAUTH_PRODUCTION_PRIVATE_BETA_SUBJECT_DIGESTS` and deploy the PR314 digest configuration.
- [ ] Re-run fresh authenticated ChatGPT `tools/list` and `tools/call` proof after the digest migration and deployment.
...
- [[sources/2026-07-13-pr313-pr316-mcp-private-beta-readiness-checkpoint]]
