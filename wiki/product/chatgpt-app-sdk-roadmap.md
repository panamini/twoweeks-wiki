...
The ChatGPT/App SDK roadmap is now a current checkpoint page, not just a future plan. The active source truth reaches PR320 at `application-os-foundation@3710e54`, after the PR305 durable private-beta connector proof, PR306-PR311 operational and protocol hardening, PR313-PR317 digest-based readiness and CI projection work, and PR320 output minimization. The digest configuration is deployed and a fresh confidential V2 connector previously re-proved token exchange, exact six-tool discovery, and one safe read-only call. Public launch and every provider/write surface remain separately gated.
...
PR320 is merged as `3710e54f869dba1d457e43b1d33403079c7e7f4f` from reviewed head `d7f590fcc8d8ede0ce799bd0b90563af1390c4ed`. It minimizes the four authenticated read-only summary tools to a closed status-only model-visible envelope with exactly `kind`, `status`, `toolName`, and `version` in runtime `structuredContent` and the advertised output schema. The optional aggregate `summary` field is removed from the model-visible projection. The exact six-tool catalog, order, names, input schemas, OAuth scope, private-beta gate, policy kernel, `search`/`fetch` fixed-catalog outputs, internal read execution, and fail-closed behavior remain unchanged. No live connector re-proof was run for PR320.
...
| PR320 | Summary output minimization | merged as `3710e54f869dba1d457e43b1d33403079c7e7f4f`; the four authenticated summary tools expose only a closed status-only model-visible envelope while OAuth, private-beta eligibility, exact six-tool catalog, search/fetch behavior, internal reads, and launch blocks remain unchanged |
...
- [x] PR320 minimizes the four authenticated summary-tool model-visible outputs to a closed status-only envelope.
- [ ] Decide the stable public catalog/submission URL before any submission rail.
