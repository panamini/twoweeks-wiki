...
| PR87.10-PR87.15A | Local/dev MCP reachability, auth architecture checkpoint, fixture-only demo, pure auth-policy boundary, auth request orchestrator boundary, local/dev auth discovery/challenge endpoint wiring, and server-only Stytch bearer verifier boundary merged | local/dev and boundary-only; production MCP, runtime verifier wiring, real OAuth/Stytch runtime calls, real handlers, and launch surfaces still blocked |
