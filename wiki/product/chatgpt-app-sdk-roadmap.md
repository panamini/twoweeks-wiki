...
### PR308 - Private-beta operational stabilization

- [ ] Add deterministic read-only smoke coverage for public metadata, unauthenticated `/mcp`, malformed OAuth requests, secret suppression, and startup diagnostics.
- [ ] Do not add tools, user-data fields, provider calls, writes, refresh tokens, billing, account-link expansion, or public access.
- [ ] Require focused tests, build/static checks, independent review, and one fresh private connector smoke before merge.

### PR309 - One authenticated read-only user-data slice, blocked pending PR308

- [ ] Start only after PR308 is merged and a separate PR309 Change Contract names one existing safe read-side capability.
