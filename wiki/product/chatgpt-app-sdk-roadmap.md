...
PR304 is merged. PR305 is draft/proof-only and documents a durable named tunnel plus a remaining ChatGPT UI activation/UI-state blocker. The prior local Vite Clerk blocker was cleared on 2026-07-06 by starting Vite with a valid `VITE_CLERK_PUBLISHABLE_KEY`; the durable host could render sign-in and reach the signed-in CV page, and a manual OAuth authorize flow returned to ChatGPT with digest-backed access-token rows present in local Convex. Final ChatGPT connector activation is still unconfirmed because the attached ChatGPT settings UI did not launch a fresh connector OAuth flow from the `Connecter` button. Do not rerun PR89, PR90, PR92, PR93, PR94, PR95, PR96, PR96.1, PR97, PR98, PR99, PR99.1, PR99.2, PR99.3, PR100, PR101, PR102, PR103, PR104, PR105, PR295, PR106, PR107, PR108, PR300, PR301, PR302, PR303, or PR304, and do not create PR88 retroactively. Provider calls, write actions, refresh tokens, account-link lifecycle expansion, and public launch remain blocked unless a later reviewed PR explicitly opens them.
...
| PR305 | Durable connector proof | draft proof at `https://mcp.twoweeks.ai/mcp`; named Cloudflare tunnel `neyssan-mcp-pr305-twoweeks-ai` proves durable route/OAuth/token/MCP/read-side ladder directly, local Clerk sign-in rendered after `VITE_CLERK_PUBLISHABLE_KEY` was loaded, but final ChatGPT UI activation remains unconfirmed because the settings UI did not launch a fresh connector OAuth flow in the attached Chrome session; not public launch |
...
- [ ] PR305 draft: durable `mcp.twoweeks.ai` proof is documented; finish UI activation from a clean ChatGPT connector settings session now that local Clerk publishable-key env is available.
