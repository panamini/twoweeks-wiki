...
PR304 is merged. PR305 is draft/proof-only and documents a durable named tunnel plus a remaining ChatGPT UI callback/token-exchange blocker. The prior local Vite Clerk blocker was cleared on 2026-07-06 by starting Vite with a valid `VITE_CLERK_PUBLISHABLE_KEY`; the durable host could render sign-in and reach the signed-in CV page, and a manual OAuth authorize flow returned to ChatGPT with digest-backed access-token rows present in local Convex. The later app fix `fb17e6cc6171f3e54baa805c39eb5e34728f5b0a` handles stale Clerk `__session` owner-binding failures by routing browser `/oauth/continue` documents through the React bridge and retrying `owner_binding_failed` bridge fetches with a Clerk bearer token. After that fix, a fresh ChatGPT UI connection attempt completed the visible twoweeks sign-in/continuation leg and returned to ChatGPT's callback with safe query keys `code,state`, but ChatGPT still showed a generic connection error and no `/oauth/token` request reached `mcp.twoweeks.ai`. Classify the remaining breakpoint as `BLOCKED_CHATGPT_UI`, not as a missing Clerk key, wildcard redirect fix, or user form-fill action. Do not rerun PR89, PR90, PR92, PR93, PR94, PR95, PR96, PR96.1, PR97, PR98, PR99, PR99.1, PR99.2, PR99.3, PR100, PR101, PR102, PR103, PR104, PR105, PR295, PR106, PR107, PR108, PR300, PR301, PR302, PR303, or PR304, and do not create PR88 retroactively. Provider calls, write actions, refresh tokens, account-link lifecycle expansion, and public launch remain blocked unless a later reviewed PR explicitly opens them.
...
| PR305 | Durable connector proof | draft proof at `https://mcp.twoweeks.ai/mcp`; named Cloudflare tunnel `neyssan-mcp-pr305-twoweeks-ai` proves durable route/OAuth/token/MCP/read-side ladder directly; local Clerk sign-in rendered after `VITE_CLERK_PUBLISHABLE_KEY` was loaded; stale Clerk `__session` owner-binding retry is fixed in `fb17e6cc6171f3e54baa805c39eb5e34728f5b0a`; current ChatGPT UI attempt returns to ChatGPT callback with `code,state` query keys but no `/oauth/token` request reaches the app, so activation is `BLOCKED_CHATGPT_UI`; not public launch |
...
- [ ] PR305 draft: durable `mcp.twoweeks.ai` proof is documented; local Clerk key and stale-cookie owner-binding blockers are fixed; remaining ChatGPT UI activation breakpoint is callback handling before `/oauth/token`, requiring ChatGPT/OpenAI-side diagnostics rather than more connector recreation.
