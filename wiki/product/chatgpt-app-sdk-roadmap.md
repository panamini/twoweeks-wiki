...
| PR87.10-PR87.16 | Local/dev MCP reachability, auth architecture checkpoint, fixture-only demo, pure auth-policy boundary, auth request orchestrator boundary, local/dev auth discovery/challenge endpoint wiring, server-only Stytch bearer verifier boundary, canonical account-link storage/index contract, server-only account-link lookup adapter, deterministic auth composition boundary, local/dev runtime composition wiring, and authoritative server-only account-link lifecycle merged | local/dev, storage, lookup, composition, local runtime wiring, and internal lifecycle only; production MCP, real OAuth/Stytch runtime calls, public account-link API, endpoint/Vite lifecycle wiring, real handlers, and launch surfaces still blocked |
