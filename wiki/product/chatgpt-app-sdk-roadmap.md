...
The roadmap has moved through the early MCP/App SDK phases and now sits around PR87 / production deployment gate reconciliation plus release stabilization. PR87.17A merged the server-only MCP OAuth authorization request boundary after PR87.16 authoritative account-link lifecycle, PR87.15D local/dev runtime auth composition wiring, PR87.15C auth composition, PR87.15B1 server-only account-link lookup adapter, PR87.15B0 canonical storage/index, PR87.15A server-only Stytch bearer verifier boundary, PR87.14B local/dev-only MCP auth discovery and challenge wiring, PR87.14A auth orchestration, PR87.13 auth metadata/policy, and the PR87.12 local/dev fixture-only MCP demo. It does not open production MCP, OAuth callback/token exchange, public account-link API, endpoint/Vite lifecycle wiring, real handlers, or real user data.

PR87.17A is the current merged boundary in the MCP chain, and PR87.17B remains the next separate reviewed slice.
...
| PR87.10-PR87.17A | Local/dev MCP reachability, auth architecture checkpoint, fixture-only demo, pure auth-policy boundary, auth request orchestrator boundary, local/dev auth discovery/challenge endpoint wiring, server-only Stytch bearer verifier boundary, canonical account-link storage/index contract, server-only account-link lookup adapter, deterministic auth composition boundary, local/dev runtime composition wiring, authoritative server-only account-link lifecycle, and MCP OAuth authorization request boundary merged | local/dev, storage, lookup, composition, local runtime wiring, internal lifecycle, and authorization-request boundary only; production MCP, real OAuth/Stytch runtime calls, public account-link API, endpoint/Vite lifecycle wiring, real handlers, and launch surfaces still blocked |
