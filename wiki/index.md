...
| [[product/chatgpt-app-sdk-roadmap\|ChatGPT/App SDK Roadmap]] | PR305 private connector, PR306 secret sync, PR307 collaborator preflight, and PR308 no-credential operational smoke are merged; PR309 remains separately unapproved | current | chatgpt-app, apps-sdk |
...
Cover-letter quality has a merged PR249 staged internal Mistral V2 gate and a green Convex staging checkpoint on `dev:neat-starfish-33`; production full GO and quality repair remain separate/not approved. The MCP/App SDK track has reached durable confidential private-beta proof in PR305, team secret-sync hardening in PR306, collaborator startup portability in PR307, and a no-credential public operational canary in PR308. PR309 may open only one separately approved authenticated read-only user-data capability; its current `application_package` candidate is planning, not approval. Provider calls, write actions, refresh tokens, production account-link lifecycle expansion, non-beta access, and public launch remain blocked; PR80B remains the safe manual handoff path while ATS authorization is pending.
