...
| [[product/chatgpt-app-sdk-roadmap\|ChatGPT/App SDK Roadmap]] | PR313-PR317 are merged; digest migration, confidential OAuth token exchange, exact six-tool discovery, and one safe read-only call are live-proven | current | chatgpt-app, apps-sdk |
...
Cover-letter quality has a merged PR249 staged internal Mistral V2 gate and a green Convex staging checkpoint on `dev:neat-starfish-33`; production full GO and quality repair remain separate/not approved. The MCP/App SDK track is merged through PR317 at `0ddcfeccef1a0f803c12b227692b85e848e1561b`. The deployed private-beta configuration uses lowercase SHA-256 subject digests, a fresh confidential V2 connector completed `/oauth/token`, ChatGPT listed exactly six read-only tools, and one safe `application_package` summary call completed without private-content capture. Provider calls, write actions, refresh tokens, production account-link lifecycle expansion, non-beta access, and public launch remain blocked; PR80B remains the safe manual handoff path while ATS authorization is pending.
