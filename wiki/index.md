...
| [[product/chatgpt-app-sdk-roadmap\|ChatGPT/App SDK Roadmap]] | MCP/App SDK PR93 merged guarded inert production route shells behind PR89 activation, PR90 operational status, and PR92 route preflight; next PR94 should connect production `/oauth/authorize` to existing ownerless pre-auth intent creation while `/oauth/callback` and `/mcp` remain inert | current | chatgpt-app, apps-sdk |
