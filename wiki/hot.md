...
- PR305 is draft/proof-only: durable `mcp.twoweeks.ai` route/OAuth/token/MCP/read-side proof passes, but ChatGPT UI activation is still blocked by missing local `VITE_CLERK_PUBLISHABLE_KEY` for Clerk sign-in.
