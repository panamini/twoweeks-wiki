...
- MCP / ChatGPT App SDK: PR87.17A/PR265 is merged as authoritative server-only MCP OAuth authorization request boundary after PR87.16 account-link lifecycle, PR87.15D runtime composition wiring, PR87.15C auth composition, PR87.15B1 lookup adapter, PR87.15B0 storage, PR87.15A verifier, PR87.14B endpoint challenge wiring, PR87.14A orchestrator, PR87.13 policy, PR87.12 fixture demo, PR87.11 architecture, and PR87.10 reachability.

## Key Active Facts

- Product truth is `twoweeks`; CVForge and ProposalForge are internal module names.
- PR87.17A adds server-only OAuth request normalization and safe intent-preservation for already-validated requests; it does not open production MCP, OAuth callback/code exchange, consent UI, token persistence, public account-link API, endpoint/Vite lifecycle wiring, real Stytch/Clerk network calls, or real user data.
- The next MCP step is PR87.17B as a separate reviewed slice; do not start PR87.17B from this checkpoint.
