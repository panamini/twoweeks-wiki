...
- Current ChatGPT connector path requires a confidential OAuth client: `local-chatgpt-client`, generated per-run secret in ChatGPT, SHA-256 digest in Vite, and `client_secret_post`. Do not use `token_endpoint_auth = none` for this path.
...
- Latest PR305 UI proof: `/oauth/continue` returned ChatGPT to the callback, but a local method/path/status-only proxy saw no `POST /oauth/token`; local Convex had a recent pending authorization code and no fresh access token. Classification remains `BLOCKED_CHATGPT_UI`.
