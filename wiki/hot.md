...
- PR305 is draft/proof-only: durable `mcp.twoweeks.ai` route/OAuth/token/MCP/read-side proof passes, and local Clerk sign-in now renders with `VITE_CLERK_PUBLISHABLE_KEY`; final ChatGPT UI activation remains unconfirmed because the attached settings UI did not launch a fresh connector OAuth flow.
