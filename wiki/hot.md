...
- MCP / ChatGPT App SDK: PR87.14A is merged as a pure MCP auth request orchestrator boundary after PR87.13 auth metadata/policy, PR87.12 fixture-only local/dev demo, PR87.11 auth/account-linking architecture, and PR87.10 reachability. PR87.8 remains blocked for production exposure. PR80B manual handoff is implemented and remains the safe delivery boundary; production MCP endpoints, production `tools/list`/`tools/call`, runtime OAuth wiring, real handlers, live submit/apply, approved-answer copy, billing, PR88, and PR89 remain blocked.
...
- PR87.14A adds a pure MCP auth request orchestrator boundary on top of PR87.13 policy/metadata and PR87.12 local/dev fixture demo; PR87.8 is still blocked for production exposure; endpoint auth wiring, runtime OAuth, and future launch-readiness need separate reviewed gates.
