...
**Agent** : Codex
**Mode** : direct-update
**Source** : PR270 merge result, GitHub PR metadata, and checkpoint request

**Pages créées** :
- `wiki/sources/2026-06-26-pr87-17c2-mcp-oauth-preauth-ownership-checkpoint.md`
...
**Points notables** :
- PR270 `PR87.17C2: add MCP OAuth pre-auth ownership decision` merged into `application-os-foundation` as `98653667421349e5c552eef7ea0dd4fbeadb2bda` on `2026-06-26T17:36:03Z`.
- The final merged head was `b3600596bbf61ab2e127e04fd42f3f85548888ad`.
- The docs-only ADR amendment added the explicit browser-storage policy to the pre-auth ownership decision and kept runtime, route, Stytch, token, account-link, production, PR88, and PR89 surfaces blocked.
- CodeRabbit status was `SUCCESS`; no actionable comments were generated.
- Semgrep cloud scan was `SUCCESS`.
- Remote `CI / js-tests` and `Playwright Tests / test` failed before runner startup with a billing/spending-limit annotation; the failure was infrastructure-level rather than a docs regression.
- PR87.17C2 records the two-phase pre-auth intent decision and the browser-storage policy, with the next slice now `PR87.17C3`.

**Open items** : Start PR87.17C3 as the next separate reviewed slice. Do not start PR87.17D, PR88, PR89, or cover-letter work from this checkpoint.
